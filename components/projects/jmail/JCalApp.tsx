...
export const JCalApp: React.FC<{ onNavigate?: (app: 'jmail' | 'jefftube' | 'jcal') => void }> = ({ onNavigate }) => {
	...
	const calendarEvents = [
		...
		{ col: 1, top: 0, height: 1, label: "Sergey's Departure", bg: 'bg-jmail-flight' },
		{
			...
			top: 4,
			height: 2,
			label: 'Deliver up-to-date budget of the office building',
			bg: 'bg-jmail-appointment'
		},
		...
		<div className="flex h-full w-full flex-col overflow-hidden rounded-xl bg-jmail-surface font-roboto text-jmail-text">
			...
			<div className="flex min-h-0 flex-1 overflow-hidden">
				...
				<div className="w-[132px] shrink-0 overflow-y-auto border-r border-jmail-border bg-jmail-surface px-2 py-2">
					...
					<div className="mb-1 flex items-center justify-between">
						...
						<div className="flex gap-2">
							<span className="cursor-pointer text-xs text-jmail-text-dimmer">‹</span>
							<span className="cursor-pointer text-xs text-jmail-text-dimmer">›</span>
						</div>
					</div>
					{/* Mini calendar */}
					<div className="mb-2">
						<div className="grid grid-cols-7">
							{miniCalDays.map((d, i) => (
								<div
									key={i}
									className="flex h-4 items-center justify-center text-center text-[9px] font-medium text-jmail-text-dimmer"
								>
									{d}
								</div>
							))}
						...
						{miniCalRows.map((row, ri) => (
							<div key={ri} className="grid grid-cols-7">
								{row.map((d, ci) => (
									<div
										key={ci}
										className={`flex h-4 items-center justify-center text-center text-[9px] ${
											d === 1 && ri === 0
												? 'h-3.5 w-3.5 rounded-full bg-jmail-primary text-white'
												: (d > 26 && ri === 0) || (d < 3 && ri === 4)
												? 'text-jmail-text-dimmer'
												: 'text-jmail-text-secondary'
										}`}
									>
										{d}
									</div>
					...
					<div className="border-t border-jmail-border pt-2">
						<div className="mb-1.5 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-jmail-text-dim">
							<span className="text-xs">▾</span> EVENT TYPES
						</div>
						<div className="mb-1 flex items-center gap-1.5 rounded bg-jmail-surface-selected-soft px-1.5 py-1">
							<div className="flex h-3 w-3 items-center justify-center rounded-sm bg-jmail-primary">
						...
						{eventTypes.map((t) => (
							<div key={t.name} className="flex items-center gap-2 px-2 py-0.5">
								<div className={`h-2.5 w-2.5 rounded-full ${t.color} shrink-0`} />
					...
					{/* People */}
					<div className="mt-2 border-t border-jmail-border pt-2">
						<div className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-jmail-text-dim">PEOPLE</div>
						<div className="flex items-center gap-1.5 rounded bg-jmail-surface-hover px-2 py-1.5">
							<svg
								className="h-3.5 w-3.5 text-jmail-text-dimmer"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
								strokeWidth={2}
				...
				<div className="flex flex-1 flex-col overflow-y-auto overflow-x-hidden">
					{/* Week header bar */}
					<div className="flex shrink-0 items-center border-b border-jmail-border px-4 py-1.5">
						<div className="flex items-center gap-2">
							<span className="cursor-pointer text-xs text-jmail-text-dimmer">‹</span>
							<span className="cursor-pointer text-xs text-jmail-text-dimmer">›</span>
						</div>
						<span className="ml-3 text-sm font-medium text-jmail-text">Dec 2006 – Jan 2007</span>
						<div className="ml-auto flex items-center gap-3">
							<span className="text-xs text-jmail-text-dim">8 events</span>
							<div className="rounded border border-jmail-border-light bg-jmail-surface px-2 py-1 text-xs font-medium text-jmail-text">
								Week ▾
							</div>
						</div>
					</div>
					...
					<div className="flex shrink-0 border-b border-jmail-border">
						...
						{weekDays.map((day, i) => (
							<div key={day} className="flex-1 border-l border-jmail-border-lighter py-1.5 text-center">
								<div className={`text-[10px] font-semibold ${i === 0 ? 'text-jmail-primary' : 'text-jmail-text-dim'}`}>
									{day}
								</div>
								<div
									className={`mx-auto text-sm font-medium ${
										i === 0
											? 'flex h-6 w-6 items-center justify-center rounded-full bg-jmail-primary text-white'
											: 'text-jmail-text'
									}`}
								>
									{weekDates[i]}
					...
					<div className="relative flex-1">
						<div className="flex">
							...
							<div className="w-[44px] shrink-0">
								{hours.map((h) => (
									<div key={h} className="flex h-[36px] items-start justify-end pr-1.5">
										<span className="text-[10px] leading-none text-jmail-text-dimmer">{h}</span>
									</div>
								))}
							</div>
							{/* Day columns */}
							<div className="relative flex flex-1">
								{weekDays.map((_, colIdx) => (
									<div key={colIdx} className="relative flex-1 border-l border-jmail-border-lighter">
										{hours.map((_, rowIdx) => (
								...
								{calendarEvents.map((evt, i) => {
									...
									return (
										<div
											key={i}
											className={`absolute ${evt.bg} overflow-hidden rounded px-1.5 py-0.5 text-white`}
											style={{
												left: `calc(${left}% + 1px)`,
										...
										>
											<span className="line-clamp-3 text-[10px] font-medium leading-tight">{evt.label}</span>
										</div>
									)
								})}
							</div>
