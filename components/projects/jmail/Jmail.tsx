		...
		<div id="jmail" className="relative">
			...
			<ProjectDescription>
				...
				<ul className="max-w-[80ch] list-disc space-y-3 pl-5">
					<li>
						<b>JeffTube</b> got over 150k likes and over 10 million views on twitter in total, here are some tweets{' '}
						<a
							href="https://x.com/whosmatu/status/2019501503581651042"
							target="_blank"
							rel="noreferrer"
						...
						and{' '}
						<a
							href="https://x.com/jmailarchive/status/2023199834128421106"
							target="_blank"
							rel="noreferrer"
							className="text-blue-500 hover:underline"
						>
			...
			<div className="relative my-3 inline-block">
				...
				<div className="relative flex rounded-md border border-black bg-white p-0.5">
					<div
						className="absolute bottom-0.5 top-0.5 rounded bg-black transition-all duration-300 ease-in-out"
					...
					{items.map((item, i) => (
						<button
							...
							className={cn(
								'relative z-10 flex cursor-pointer items-center gap-1.5 rounded px-3 py-1 text-sm font-medium capitalize transition-colors duration-300',
			...
			<div className="mt-32 h-[500px] w-full">
				{sorted.map((item, index) => (
					<div
						key={item}
						className={cn(
							'absolute bottom-0 left-0 right-0 h-[500px] w-full rounded-xl border shadow-sm transition-transform delay-[200ms] duration-500',
							index === 0 && 'z-[3]',
